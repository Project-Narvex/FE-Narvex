export interface BlogArticle {
  ...
  excerpt: string;
  content?: string;
  category: 'company-updates' | 'industry-insights' | 'project-stories' | 'tips-tricks' | 'company-news';
  author: string;
  authorId?: string;
  publishDate: string;
  lastModified?: string;
...
Dengan dukungan dari 4 partner companies (Skywork.id, Gutama Learning, CreativeWork, dan Evervow.wo), Narvex mampu memberikan solusi end-to-end yang tidak hanya mencakup event management, tetapi juga creative services, educational programs, dan specialized services.

"Kami melihat peluang besar di industri MICE Indonesia. Dengan pengalaman yang telah kami miliki dan dukungan ekosistem company yang kuat, kami yakin dapat memberikan value yang signifikan bagi klien," ujar Ahmad Rizki Pratama, CEO Narvex.
...
Narvex menargetkan untuk menangani 100+ event MICE dalam 2 tahun ke depan, dengan fokus pada corporate clients, government institutions, dan international organizations yang beroperasi di Indonesia.
    ...
    seoDescription: 'Narvex meluncurkan layanan MICE komprehensif untuk pasar Indonesia dengan dukungan 4 partner companies.',
    views: 1250,
    likes: 89
  },
  {
...
export const getCategoryStats = () => {
  const categories = {
    ...
    'company-news': blogArticles.filter(a => a.category === 'company-news' && a.published).length
  };
  return categories;
};